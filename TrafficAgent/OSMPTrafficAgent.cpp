...
fmi2Status COSMPTrafficAgent::doInit() {
    ...
    this->agentModel = std::make_shared<FZIDriver::TrafficAgent>();
...
fmi2Status COSMPTrafficAgent::doCalc(
    ...
    /* Update Trajectory */
    agentModel->step(communicationStepSize, currentViewIn, currentCommandIn, currentOut);
...
#if TARGET_OPEN_PASS
            // ToDo: Find a better position to put this?
            if (need_refresh &&
